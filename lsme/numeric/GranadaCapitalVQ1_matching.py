...
class GranadaVQ1MatchingResult(matchingresult.GenericMatchingResult):
    def __init__(self, name='VQ1'):
        super().__init__(name)
        self.MVQ1 = 1
        self.gulVQ1 = np.ones((3, 3))
        self.gulVQ1bar = np.ones((3, 3))
        self.gdqVQ1 = np.ones((3, 3))
        self.gdqVQ1bar = np.ones((3, 3))
        self.exotic_params = ['MVQ1', 'gulVQ1', 'gulVQ1bar', 'gdqVQ1', 'gdqVQ1bar']
        self.nonvanishing = ['alphaOlu', 'alphaOlubar', 'alphaOqd1', 'alphaOqd1bar', 'alphaOqd8', 'alphaOqd8bar']
    ...
    def alphaOlu(self, mif1,mif2,mif3,mif4):
        return (self.MVQ1)**(-2) * self.gulVQ1[mif4,mif2] * self.gulVQ1bar[mif3,mif1]

    def alphaOlubar(self, mif1,mif2,mif3,mif4):
        return (self.MVQ1)**(-2) * self.gulVQ1[mif3,mif1] * self.gulVQ1bar[mif4,mif2]

    def alphaOqd1(self, mif1,mif2,mif3,mif4):
        return 2/3 * (self.MVQ1)**(-2) * self.gdqVQ1[mif3,mif1] * self.gdqVQ1bar[mif4,mif2]

    def alphaOqd1bar(self, mif1,mif2,mif3,mif4):
        return 2/3 * (self.MVQ1)**(-2) * self.gdqVQ1[mif4,mif2] * self.gdqVQ1bar[mif3,mif1]

    def alphaOqd8(self, mif1,mif2,mif3,mif4):
        return -2 * (self.MVQ1)**(-2) * self.gdqVQ1[mif3,mif1] * self.gdqVQ1bar[mif4,mif2]

    def alphaOqd8bar(self, mif1,mif2,mif3,mif4):
        return -2 * (self.MVQ1)**(-2) * self.gdqVQ1[mif4,mif2] * self.gdqVQ1bar[mif3,mif1]
