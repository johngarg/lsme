...
class GranadaVHMatchingResult(matchingresult.GenericMatchingResult):
    def __init__(self, name='VH'):
        super().__init__(name)
        self.MVH = 1
        self.gqVH = np.ones((3, 3))
        self.gqVHbar = np.ones((3, 3))
        self.exotic_params = ['MVH', 'gqVH', 'gqVHbar']
        self.nonvanishing = ['alphaOqq1', 'alphaOqq1bar', 'alphaOqq3', 'alphaOqq3bar']
    ...
    def alphaOqq1(self, mif1,mif2,mif3,mif4):
        return (-3/16 * (self.MVH)**(-2) * self.gqVH[mif1,mif4] * self.gqVH[mif3,mif2] + -3/16 * (self.MVH)**(-2) * self.gqVH[mif2,mif3] * self.gqVH[mif4,mif1])

    def alphaOqq1bar(self, mif1,mif2,mif3,mif4):
        return (-3/16 * (self.MVH)**(-2) * self.gqVH[mif1,mif4] * self.gqVH[mif3,mif2] + -3/16 * (self.MVH)**(-2) * self.gqVH[mif2,mif3] * self.gqVH[mif4,mif1])

    def alphaOqq3(self, mif1,mif2,mif3,mif4):
        return (1/16 * (self.MVH)**(-2) * self.gqVH[mif1,mif4] * self.gqVH[mif3,mif2] + 1/24 * (self.MVH)**(-2) * self.gqVH[mif1,mif2] * self.gqVH[mif3,mif4] + 1/16 * (self.MVH)**(-2) * self.gqVH[mif2,mif3] * self.gqVH[mif4,mif1] + 1/24 * (self.MVH)**(-2) * self.gqVH[mif2,mif1] * self.gqVH[mif4,mif3])
